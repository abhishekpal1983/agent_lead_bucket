...
/* Today's list, as every surface must see it.
   ...
   Revenue Command and the floor's own page both read this one function. That is the whole point of it: two surfaces computing the same
...
/* Walking 31,000 leads is cheap, but Overview draws four teams and the floor's own page
   asks for the same thing, so the same walk can happen a dozen times a minute. Held for
   fifteen seconds, keyed on everything that can change it. */
...
const MEMO_PATHS = ["/api/callnow2", "/api/callnow2/agents", "/api/callnow2/leads",
  "/api/callnow2/assign", "/api/vp", "/api/callnow2/wa",
...
/* The one surviving piece of the idle tracker. FreJun logs the dial and the agent then
   writes the same call up by hand, so HubSpot holds two records for one conversation.
   The Loop WA call counts would read nearly half as high again as the truth without
   this, which is why the config outlived the views it was written for. */
const IDLE_CFG = { dedupeMs: parseInt(process.env.CALL_DEDUPE_MS || "120000", 10) };
...
function authGate(req, res, next){
  ...
  if (s.role === "agent") {
    ...
    if (p.indexOf("/api/") === 0) {
      ...
      // so without this an agent could read a colleague's drill or owner record just by
      // editing the URL.
      const OWNED = [/^\/api\/agent\/([^\/]+)/, /^\/api\/drill\/([^\/]+)/,
        /^\/api\/callnow\/refresh-owner\/([^\/]+)/];
      ...
      // Manager and VP surfaces are closed to agents outright. The talktime report is
      // not one of them: it is scoped to the agent's own day by design.
      if (p.indexOf("/api/vp") === 0) {
...
app.post("/api/org/import", express.json({ limit: "4mb" }), function(req, res){
  ...
  // Only the setup travels. Anything a running service produced for itself must not be
  // overwritten by a copy from somewhere else.
  ORG.teams = b.teams;
...
/* The creator planner was removed on 31 August 2026 at the user's request: the pages
   ...
   that only they used, and the twelve hourly sync that walked every contact for nine
   creators to feed them.

   Its data files may still be sitting on the volume as plan_data.json, plan_prefs.json and
   plan_state.json. Nothing reads them now. They are left alone rather than deleted, because
   throwing away somebody's saved targets on a tidy-up is not a decision code should make
   on its own. */
