...
/* The other direction, for walking a span of days. Parsed as UTC midnight on purpose:
   adding 86,400,000 to a local-midnight date lands on the wrong day twice a year in any
   timezone with daylight saving, and India not having it is not a reason to write code
...
/* Whether a cached build of a day may still be used.

   The rule this replaces was `if (dayKey < today) return hit`, commented "a past day cannot
   change". The day cannot. The cache entry can be a picture of the afternoon, and the
   moment midnight passes that test starts calling it final.

   It froze 14 September: the lock fires just after midnight, was served a build made during
   the evening, and missed every call logged after it. The recheck rebuilt properly the next
   day, found 84 calls that had been there all along, and reported them as amendments
   against the agents. Nobody had touched anything.

   So an entry built while its own day was still running is never final. */
function cacheUsable(hit, dayKey, today, opts){
  const o = opts || {};
  if (!hit || hit.error) return false;
  if (hit.builtOn === dayKey && dayKey < today) return false;   // built mid-day, day now over
  /* No build day recorded means we cannot tell when it was made, and the whole point here
     is not guessing. Rebuilding costs one read; trusting it wrongly costs a published day. */
  if (!hit.builtOn && dayKey < today) return false;
  if (dayKey < today) return true;                              // built after the day closed
  const ttl = o.ttlMs == null ? 900000 : o.ttlMs;
  const now = o.now == null ? Date.now() : o.now;
  return (now - (hit.at || 0)) < ttl;
}

module.exports = { pendingLock, prevDay, nextDay, snapshotOf, diff, byOwner, cacheUsable };
