...
/* The day's list follows assignment.
   ...
   The frozen list is written once, early, and the rule used to be that a lead counted
   against whoever held it that morning whatever happened afterwards. That protected an
   agent's denominator from moving under them, but it also meant a lead handed to somebody
   at eleven never appeared on their screen, which is the opposite of useful.

   So assignment now moves a lead, in all three shapes it comes in: a lead created and
   assigned after the list froze, a lead that sat unassigned and was picked up, and a lead
   taken from one agent and given to another. The first two only add. The third moves, and
   the old agent's count drops by one, which is a real change from before: it is reported
   rather than left to be noticed, and any calls already made stay credited to whoever made
   them, because calls are counted from call records and not from this list. */
function assignBase(base, liveAll, opts){
  const since = (opts && opts.since) || 0;
  const countable = (opts && opts.countable) || function(){ return true; };
  const qualifies = (opts && opts.qualifies) || function(){ return true; };
  const pack2 = (opts && opts.pack) || pack;
  const classify2 = (opts && opts.classify) || null;
  const out = {};
  Object.keys(base).forEach(function(id){ out[id] = base[id]; });
  let added = 0, moved = 0;
  Object.keys(liveAll || {}).forEach(function(id){
    const r = liveAll[id];
    if (!r || !r.owner) return;
    // Only leads assigned today, and only to somebody who is actually working.
    if (!r.assignedAt || r.assignedAt < since) return;
    if (!countable(String(r.owner))) return;
    const had = out[id] ? read(out[id]) : null;
    if (!had) {
      if (!qualifies(r)) return;                 // not list-worthy on its own merits
      if (!classify2) return;
      out[id] = pack2(classify2(r));
      added++;
      return;
    }
    if (String(had.owner) === String(r.owner)) return;
    /* Stored as an object rather than packed, the way promotion does it. The packed line
       carries a fixed set of columns and would drop the moved flag, which is the one
       thing the page needs in order to explain a number that changed during the day. */
    out[id] = Object.assign({}, had, {
      owner: String(r.owner), counted: true, moved: true,
      why: Object.assign({}, had.why, { needs: false })
    });
    moved++;
  });
  return { base: out, added: added, moved: moved };
}
...
module.exports = {
  ...
  read, promote, promoteBase, correctBase, assignBase,
