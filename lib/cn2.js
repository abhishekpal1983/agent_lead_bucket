...
const REASONS = ["form", "score", "intl", "fresh", "refill", "ifc", "over", "nofu"];
...
function pack(c){
  ...
  return [c.stage, c.sec, c.t, mask, c.owner, c.creator, c.source || ""].join("|");
}
function unpack(v){
...
const COLUMNS = ["form", "score", "intl", "fresh", "refill", "ifc", "any", "all"];
...
function aggregate(base, live, day, stageOrder){
  ...
  Object.keys(base).forEach(function(id){
    ...
    if (!byAgent[aid]) byAgent[aid] = { n: cell(), a: cell(), d: cell(), offBase: 0, offBaseCalls: 0 };
  ...
  return { sections: sections, totals: totals, movement: move, byAgent: byAgent };
}
