...
function talkFor(calls, opts){
  ...
  const rows = calls || [];
  /* Recorded, not non-zero. FreJun writes a duration of 0 on a dial that rang out, and
     that is a measurement: we know it lasted no time. A manual log carries no duration
     property at all, and that is ignorance. Treating the two the same marked most of the
     floor's calls as unrecorded, because most dials are unanswered. `hasDur` is the
     property's presence; older callers that do not set it fall back to a positive
     duration, which is the previous behaviour. */
    const known = rows.filter(function(c){
    return c.hasDur === undefined ? (c.durMs || 0) > 0 : !!c.hasDur; });
  ...
  return {
    ...
    known: known.length,
    ms: ms,
    unknown: rows.length > 0 && known.length === 0,
    none: rows.length === 0,
    short: known.length > 0 && ms < minMs
  };
}
