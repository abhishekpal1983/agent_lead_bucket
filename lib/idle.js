...
/* The note logged after the call it describes.
   ...
   A manual record that carries a declared length, or that the agent typed as a WhatsApp
   call, is never merged. Both are the agent saying "this was its own conversation", so the
   rule gets stricter as those habits spread rather than looser.
...
function writeUps(records, cfg){
  ...
  const out = [], absorbed = [];
  rows.forEach(function(r){
    /* `isWa` is the agent marking the call as one they dialled by hand. That is a direct
       statement that it was its own conversation, so it outranks every inference here. */
    const orphan = r.contact && !r.hasDur && !(r.declaredMs > 0) && !(r.noteMs > 0) && !r.isWa;
